...
- [ ] **Usuario de base de datos con permisos mínimos.** Hoy el entorno local usa `root` (contraseña `root123` en el `.env`). **Ya está preparado:** ejecutar `database/crear_usuario_app.sql` en el servidor (solo concede `SELECT, INSERT, UPDATE, DELETE`; no toca datos), cambiando antes la contraseña de la plantilla, y poner ese usuario en `DB_USER`/`DB_PASS`. La estructura se importa siempre con el administrador. **Verificado (2026-09-21)** contra la base local con un usuario temporal `SELECT/INSERT/UPDATE/DELETE`: la base rechaza `CREATE/DROP/ALTER/TRUNCATE` y todas las suites pasan (aislamiento 20, ministerio 13, gestión 23, mantenimiento 19, navegador 91, js-formulario 7, js-externo 20, sin-cdn 13). Eso encontró y corrigió un fallo: si faltaba el índice `idx_fecha`, el cron de purga abortaba al no poder hacer `ALTER`; ahora avisa en el log y sigue purgando. Pendiente: ejecutar el script en el servidor real (allí solo se prueba que conecte).
